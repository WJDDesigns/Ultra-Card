              ...
              ? html`
                  <div style="display:flex;gap:8px;margin-top:12px;">
                    <input
                      ...
                      style="flex:1;padding:8px 12px;border:1px solid var(--divider-color);border-radius:var(--uc-r-8, 8px);background:transparent;color:${text};font-size:13px;outline:none;"
                    ...
                    />
                    <button
                      style="flex-shrink:0;width:36px;height:36px;border:none;border-radius:var(--uc-r-8, 8px);background:${accent};color:var(--text-primary-color,#fff);cursor:pointer;display:flex;align-items:center;justify-content:center;"
  ...
  private _renderItem(
    ...
    return html`
      <div style="display:flex;align-items:center;gap:10px;padding:6px 2px;">
        <button
          style="flex-shrink:0;width:20px;height:20px;border-radius:var(--uc-r-6, 6px);border:2px solid ${done ? o.accent : 'var(--divider-color)'};background:${done ? o.accent : 'transparent'};cursor:${canUpdate ? 'pointer' : 'default'};display:flex;align-items:center;justify-content:center;padding:0;"
          ?disabled=${!canUpdate}
          @click=${() => {
            if (!canUpdate) return;
