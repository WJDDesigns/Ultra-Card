  ...
  renderPreview(module: CardModule, hass: HomeAssistant, config?: UltraCardConfig): TemplateResult {
    ...
    if (style === 'segmented') {
      return this.wrapWithAnimation(html`
        <style>
          .sel-seg-${mid} { display:flex; border-radius:var(--uc-r-8, 8px); overflow:hidden; border:1px solid var(--divider-color); }
    ...
    if (style === 'chips') {
      return this.wrapWithAnimation(html`
        <style>
          ...
          .sel-chip-${mid} { padding:8px 16px; border-radius:var(--uc-r-20, 20px); border:1px solid var(--divider-color);
    ...
    return this.wrapWithAnimation(html`
      <style>
        ...
        .sel-dd-${mid} { width:100%; padding:12px; font-size:${fontSize}px; color:${textColor}; font-family:inherit;
          background:transparent; border:1px solid var(--divider-color); border-radius:var(--uc-r-8, 8px); cursor:pointer;
