...
// editor (and wiping the fallback message) on every re-render. Keyed by card
// type so swapping the card re-evaluates instead of inheriting an old verdict.
const editorUnavailable = new Map<string, string>();
...
function hasRenderedContent(element: HTMLElement): boolean {
  ...
  const roots: ParentNode[] = element.shadowRoot ? [element.shadowRoot, element] : [element];
  return roots.some(root => Array.from(root.children).some(child => child.tagName !== 'STYLE'));
}

/**
 * True only when the container is actually on screen.
 *
 * A collapsed section, a background tab or a detached panel all report zero size
 * and no paint, which is indistinguishable from an editor that refused to render.
 * Declaring "no visual editor" from that evidence is how a working editor gets
 * permanently replaced by the fallback message.
 */
function canJudgeRendering(container: Element): boolean {
...
export class UltraNativeCardModule extends BaseUltraModule {
  ...
  renderGeneralTab(
    ...
    const setupEditor = (container: Element | undefined) => {
      ...
      if (editorUnavailable.get(module.id) === module.card_type) {
        ...
        (async () => {
          ...
          const checkEditorRendered = async () => {
            for (const delay of [250, 500, 1000, 2000]) {
              ...
              if (!canJudgeRendering(container)) return;
            }
            console.warn(
          ...
          // The budget only starts once setConfig has run. Timing it from here
          // instead would race the lazy-chunk load that initializeEditor awaits,
          // and a slow load would be misread as an editor that cannot render.
          initializeEditor()
            .catch(error => {
              console.error('[UC Native Card] Editor initialization failed:', error);
            })
            .then(checkEditorRendered);
          ...
          const showNotFound = () => {
            ...
            container.innerHTML = `
              <div style="padding: 16px; text-align: center; color: var(--error-color);">
                ...
                <p>Card not found: ${escapeHtml(module.card_type)}</p>
              </div>
            `;
          };

          ucNativeCardsService
            .ensureCardElementLoaded(module.card_type)
            .then(loaded => {
              if (!container.isConnected) return;
              if (loaded) {
                setupCard(container);
            ...
            .catch(error => {
              console.error('[UC Native Card] Failed to load card element:', error);
              showNotFound();
            });
          return;
        }
