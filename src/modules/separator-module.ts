...
const separatorSettings = createLazySettings(
  () => import(/* webpackChunkName: "core-settings" */ './settings/separator-module-settings'),
  'separator settings'
);
...
export class UltraSeparatorModule extends BaseUltraModule {
  ...
  preloadSettings(): Promise<void> {
    return separatorSettings.prefetch();
  }
  ...
  renderGeneralTab(
    ...
    return separatorSettings(s =>
      s.renderSeparatorGeneralTab(this, module, hass, config, updateModule)
    );
  }
      ...
      html`
        <div
          ...
          style="${designStyles}; ${this.styleObjectToCss(
            containerStyles
          )}; cursor: ${(separatorModule.tap_action &&
        ...
        >
          <div
            class="separator-preview ${separatorModule.orientation === 'vertical'
              ? 'vertical'
              : ''}"
