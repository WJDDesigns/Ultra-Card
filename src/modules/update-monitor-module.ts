            ...
            ${m.show_title !== false
              ? html`
                  ...
                  >
                    ...
                    <span
                      style="font-size:12px;font-weight:700;padding:2px 10px;border-radius:var(--uc-r-10, 10px);background:${pending > 0 ? cUpd : cOk}22;color:${pending > 0 ? cUpd : cOk};"
                    >
                      ${pending > 0
    ...
    return html`
      ...
      >
        ${m.show_entity_picture !== false && r.picture
          ? html`<img
              src="${r.picture}"
              style="width:28px;height:28px;border-radius:var(--uc-r-6, 6px);object-fit:contain;flex-shrink:0;"
            />`
          : html`<ha-icon
          ...
          : r.hasUpdate && m.show_install_button !== false && r.canInstall
            ? html`
                <button
                  ...
                  style="flex-shrink:0;padding:6px 12px;border:none;border-radius:var(--uc-r-8, 8px);background:${col};color:var(--text-primary-color,#fff);font-weight:600;font-size:12px;cursor:pointer;"
                  @click=${(ev: Event) => {
                    ev.stopPropagation();
