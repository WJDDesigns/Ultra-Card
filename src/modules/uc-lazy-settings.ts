  ...
  return html`
    <div class="uc-settings-loading" aria-busy="true" style="padding: 8px 0;">
      ${[70, 100, 55].map(
        w => html`
          <div
            style="height: 14px; width: ${w}%; margin: 12px 0; border-radius: 6px; background: var(--divider-color, rgba(127,127,127,0.25)); opacity: 0.6;"
          ></div>
        `
      )}
    </div>
...
function renderFailed(): TemplateResult {
  return html`
    <div
      ...
      style="padding: 12px; border-radius: 8px; background: var(--error-color, #db4437); color: #fff; font-size: 13px;"
    >
