...
const barSettings = createLazySettings(
  () => import(/* webpackChunkName: "core-settings" */ './settings/bar-module-settings'),
  'bar settings'
);
...
export class UltraBarModule extends BaseUltraModule {
  ...
  protected _templateInputDebounce: any = null;
   ...
   */
  protected normalizeBoolean(value: unknown, defaultValue: boolean): boolean {
  ...
  renderGeneralTab(
    ...
    return barSettings(s => s.renderBarGeneralTab(this, module, hass, config, updateModule));
  }
  ...
  renderPreview(
    ...
    const needsEntity =
      pctType !== 'time_progress' && pctType !== 'range' && pctType !== 'difference';
    ...
    if (pctType === 'range') {
      ...
      if (!rangeStartEntity || !rangeEndEntity) {
        return this.renderGradientErrorState(
          ...
          localize(
            'editor.bar.error_no_range_desc',
            lang,
    ...
    if (pctType === 'difference') {
      ...
      const missingEntities = [currentEntity, totalEntity].filter(
    ...
    const resolveMinMax = (): { min: number | undefined; max: number | undefined } => {
      ...
      const allowUnifiedRange = barModule.unified_template_mode && pctType !== 'difference';
    ...
    if (
      isPreviewMode &&
      (!barModule.unified_template_mode || (pctType === 'difference' && !hasDifferenceEntities))
    ) {
    ...
    if (barModule.limit_entity && hass?.states[barModule.limit_entity]) {
      ...
      if (pctTypeForLimit === 'difference') {
        ...
        if (limitManualMin !== undefined || limitManualMax !== undefined) {
          limitPercentage = calculatePercentageWithRange(
            limitValue,
            0,
            total,
            limitManualMin,
    ...
    const renderBarScaleTicks = (): TemplateResult => {
      ...
      const scaleLabelColor = (barModule as any).scale_label_color || 'var(--secondary-text-color)';
        ...
        >
          ${ticks.map((tick, index) => {
            ...
            const hideForDensity =
              mobileReduceDensity && index !== 0 && index !== ticks.length - 1 && index % 2 === 1;
            ...
            return html`
              ...
              >
                ...
                ${scaleShowLabels && displayLabel !== null && !hideForDensity
                  ? html`
                      <span
                        class="scale-label ${isEdgeStart ? 'edge-start' : ''} ${isEdgeEnd
                          ? 'edge-end'
                          : ''}"
    ...
    return this.wrapWithAnimation(
      ...
      >
        ...
        >
          ...
          ${
            pctType !== 'difference' && !hass?.states[barModule.entity] && barModule.entity
              ? html`
                  <div
                    class="entity-error"
  ...
  private resolveBarAnimationDirection(
    barModule: BarModule,
    hass?: HomeAssistant
  ): 'normal' | 'reverse' {
  ...
  protected getBarSizeFromHeight(height: number): 'thin' | 'regular' | 'thick' | 'thiccc' {
  ...
  protected getBarRadiusFromStyle(borderRadius: number): 'round' | 'square' | 'rounded-square' {
