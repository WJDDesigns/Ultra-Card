...
export const INFO_TEMPLATE_KEYS = [
  'icon',
  'icon_color',
  'name',
  'name_color',
...
const infoSettings = createLazySettings(
  () => import(/* webpackChunkName: "core-settings" */ './settings/info-module-settings'),
  'info settings'
);
...
export class UltraInfoModule extends BaseUltraModule {
  ...
  private _buildUnifiedInfoTemplateKey(
    ...
    const cardScope =
      ...
      String((cardConfig as any).__ucInstanceId).trim() !== ''
        ...
        : 'card_unknown';
    const moduleScope =
      infoModule.id && infoModule.id.trim() !== '' ? infoModule.id : 'module_unknown';
    const entityScope =
  ...
  renderGeneralTab(
    ...
    return infoSettings(s => s.renderInfoGeneralTab(this, module, hass, config, updateModule));
  }
  ...
  renderPreview(
    ...
    if (!infoModule.info_entities || infoModule.info_entities.length === 0) {
      return this.renderGradientErrorState(
        ...
        localize(
          'editor.info.error_no_entities_desc',
          lang,
    ...
    return this.wrapWithAnimation(
      html`
        ...
        <div
          ...
          style="${designStyles}; ${this.styleObjectToCss(
            containerStyles
          )}; align-self: ${gridAlignment === 'left'
            ? 'flex-start'
            : gridAlignment === 'right'
              ? 'flex-end'
              : 'center'};"
            ...
            >
              ${validEntities.slice(0, 3).map((originalEntity, index) => {
                ...
                if (entityState) {
                  if (
                    entity.attribute &&
                    entityState.attributes?.[entity.attribute] !== undefined
                  ) {
                ...
                const getTextShadow = () => {
                  ...
                  const blur =
                    designProperties.text_shadow_blur || moduleWithDesign.text_shadow_blur;
                  const color =
                ...
                const isHorizontalWithDistribution =
                  nameValueLayout === 'horizontal' && contentDistribution !== 'normal';
                const hasIcon =
                  entity.show_icon !== false &&
                ...
                const contentElement = html`
                  <div
                    ...
                    justify-content: ${isHorizontalWithDistribution && !hasIcon
                      ? contentDistribution
                      : 'flex-start'};
                    flex: ${isHorizontalWithDistribution && !hasIcon
                      ? '1'
                ...
                const iconNameGroupElement =
                  isHorizontalWithDistribution && hasIcon
                    ? html`
                        <div
                      ...
                      "
                        >
                          ${iconPosition === 'left'
                            ? html`${iconElement}${nameElement}`
                            : html`${nameElement}${iconElement}`}
                        </div>
                      `
                    : null;
                ...
                const element = ((m: any) =>
                  ...
                  "
                      ...
                      @touchstart=${(e: Event) =>
                        this.handleTouchStart(e, infoModule, hass, config)}
  ...
  // Helper method to get available attributes from an entity
  protected _getEntityAttributes(
    entityId: string,
    hass: HomeAssistant
  ): { value: string; label: string }[] {
    const options = [{ value: '', label: 'None (Use Entity State)' }];
