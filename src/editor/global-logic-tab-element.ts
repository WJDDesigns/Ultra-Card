...
import type { GlobalLogicArgs, GlobalLogicTarget } from '../tabs/global-logic-tab';
...
/**
 * `<ultra-global-logic-tab>`: the Logic settings tab mounted by
 * `GlobalLogicTab.render()` (src/tabs/global-logic-tab.ts).
 *
 * Lives in the editor chunk so the ~40 KB of form markup stays out of
 * ultra-card.js. Renders into light DOM on purpose: the markup relies on the
 * layout tab's stylesheet exactly as it did when it was inlined there. `args`
 * is a fresh object on every parent render, so this re-renders whenever the
 * parent does, matching the previous inline behavior.
 */
@customElement('ultra-global-logic-tab')
export class UltraGlobalLogicTab extends LitElement {
  @property({ attribute: false }) public args?: GlobalLogicArgs;
  ...
  protected override render(): TemplateResult | typeof nothing {
    const a = this.args;
    if (!a) return nothing;
    return renderGlobalLogicTab(a.target as any, a.hass, a.update as any, a.logicTarget);
  }
}

declare global {
  interface HTMLElementTagNameMap {
    'ultra-global-logic-tab': UltraGlobalLogicTab;
  }
}

export function renderGlobalLogicTab<M extends CardModule | CardRow | CardColumn>(
  module: M,
  hass: HomeAssistant,
