...
/**
 * `<ultra-global-actions-tab>`: the Actions settings tab every module mounts
 * through `GlobalActionsTab.render()` (src/tabs/global-actions-tab.ts).
 *
 * Defined here, in the editor chunk, rather than next to the static shim so the
 * ~50 KB of form code never ships in ultra-card.js. Modules only render the
 * tag; it upgrades once the editor is loaded.
 */
@customElement('ultra-global-actions-tab')
export class UltraGlobalActionsTab extends LitElement {
...
declare global {
  interface HTMLElementTagNameMap {
    'ultra-global-actions-tab': UltraGlobalActionsTab;
  }
}
