 ...
 * WordPress presets are fetched from ultracard.io
 */
class UcPresetsService {
  ...
  /**
   * Load WordPress presets from API
   */
  private async _loadWordPressPresets(force = false): Promise<void> {
    ...
    try {
      console.log('Loading WordPress presets from ultracard.io...');
      ...
      console.log(`Successfully loaded ${this._wordpressPresets.length} WordPress presets`);

      // Notify listeners of new presets
      this._notifyListeners();
    } catch (error) {
      console.error('Failed to load WordPress presets:', error);
